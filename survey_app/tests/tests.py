...
def test_pages_with_db(auto_login_user):
    ...
    def add_question_by_post(surv):
        ...
        response = client.post(
            reverse("survey_app:add_question", kwargs={"survey_id": surv.id}),
            context,
            follow=True,
        )
        count = Question.objects.all().count()
    ...
    def test_2url_list(user2):
        ...
        response = client.get(
            reverse("survey_app:answer", kwargs={"survey_id": survey.id}), follow=True
        )
        assert response.status_code == 200
        ...
        response = client.get(
            reverse("survey_app:results", kwargs={"survey_id": survey.id}), follow=True
        )
        assert response.status_code == 404
        ...
        context = dict(choice="-choice1")
        response = client.post(
            reverse("survey_app:add_choice", kwargs={"question_id": question_id}),
            context,
            follow=True,
        )
        assert response.status_code == 404
        ...
        response = client.get(
            reverse("survey_app:question_detail", kwargs={"question_id": question_id}),
            follow=True,
        )
        assert response.status_code == 404
