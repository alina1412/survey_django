...
from .models import Answer, Choice, Question, Survey, User
...
def get_qset_questions_of_survey(survey_id):
    return Question.objects.select_related("survey").filter(survey__id=survey_id)


def get_owned_surveys(user_id):
    return Survey.objects.select_related("owner").filter(owner__id=user_id)
...
def create_demo_user_survey(user):
    ...
    print(survey)
    questions = [
        ("What programming language do you learn?", ("python", "Java", "C++", "other")),
        (
